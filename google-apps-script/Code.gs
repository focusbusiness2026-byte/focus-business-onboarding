...
function valueFor(header, data) {
  const fields = {
    ...
    "Preguntas adicionales": () => data.additionalLeadQuestions, "Objetivo campaña": () => data.campaignObjective, "Conversión campaña": () => data.campaignConversion, "Público campaña": () => data.campaignAudience, "Destino campaña": () => data.campaignDestination, "Objetivo landing": () => data.landingGoal, "Contenido landing": () => data.landingSections, "Uso VSL": () => data.landingVslChoice, "Enlace VSL": () => data.landingVslUrl, "Indicaciones VSL": () => data.landingVslNotes, "Herramientas actuales": () => asText(data.toolsInUse), "Herramientas a conectar": () => asText(data.toolsToConnect),
