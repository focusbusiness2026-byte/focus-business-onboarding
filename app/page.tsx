  ...
  return <FieldContext.Provider value={{ data, errors: fieldErrors, setValue, toggle }}><main>
    ...
    <section className="content"><header><div><p className="eyebrow">PASO {step + 1} DE {steps.length}</p><div className="progress"><i style={{ width: `${completion}%` }} /></div></div></header>
      <form onSubmit={submit} noValidate className="card">
        ...
        {step === 3 && <details className="setup-disclosure domain-disclosure"><summary>Dominio o subdominio para páginas, formularios y portal</summary><div className="purpose-note"><strong>Opcion 1 · recomendada: gestionarlo con Focus Business</strong><p>Indica el nombre deseado y una alternativa. Comprobaremos disponibilidad real y, si procede, prepararemos las instrucciones. El dominio también puede servir para correos empresariales, por ejemplo informacion@tuproductora.com.</p></div><div className="grid three"><Select field="subdomainSetup" label="¿Qué quieres hacer?" items={["No necesito dominio o subdominio", "Comprar un dominio con Focus Business", "Crear un subdominio en mi dominio", "Usar un subdominio existente", "Necesito ayuda para decidir"]} /><Select field="subdomainPurpose" label="¿Para qué se utilizará?" items={["Portal / CRM", "Formularios", "Calendario de reservas", "Landing pages", "Portafolio audiovisual", "Otro"]} /><Select field="subdomainOwnership" label="¿Quién controla el dominio principal?" items={["La empresa", "Un proveedor o agencia", "Focus Business", "No lo sé"]} /></div><div className="grid two"><Text field="subdomainPreferred" label="Dominio o subdominio preferido" placeholder="Ej. productoranorte.com o landing.productoranorte.com" /><Text field="subdomainAlternative" label="Alternativa" placeholder="Ej. audiovisualproductoranorte.com" /></div><div className="grid two"><Select field="subdomainDnsStatus" label="Estado del acceso para configurar DNS" items={["Podemos modificar los DNS", "Debe hacerlo un proveedor", "Necesitamos instrucciones", "No lo sé"]} /><Select field="subdomainVerification" label="Estado de verificación" items={["Pendiente de comprobar disponibilidad", "Disponible, pendiente de DNS", "DNS configurado, pendiente de verificar", "Verificado", "No aplica"]} /></div><details className="domain-own-option"><summary><span>Opción 2 · utilizar tu propio dominio registrando un subdominio</span><b>NO RECOMENDADO</b></summary><div><p>Esta opción requiere configurar DNS manualmente en tu registrador y puede ser más compleja.</p><ul><li>Crear el subdominio, por ejemplo <code>landing.tuproductora.com</code>.</li><li>Apuntar el registro CNAME al host que se indique.</li><li>Verificar el dominio mediante un registro TXT.</li><li>Si se usará para envío de correo, revisar SPF, DKIM y DMARC.</li><li>Esperar la propagación DNS, que puede tardar de minutos a horas.</li><li>Los pasos concretos varían según el proveedor.</li></ul><p><strong>Recomendación:</strong> evita esta opción si buscas rapidez y menos fricción técnica.</p></div></details><p className="intro">La consulta de disponibilidad y la verificación se realizan de forma real después de revisar estos datos. El formulario no compra dominios, no simula disponibilidad y no cambia DNS automáticamente. No escribas contraseñas ni claves del registrador.</p></details>}
