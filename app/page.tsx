...
export default function Home() {
  ...
  const downloadConfiguration = () => {
    ...
    link.href = url;
    link.download = `focus-onboarding-${String(data.companyName || "productora").toLowerCase().replace(/[^a-z0-9]+/g, "-")}.json`;
    link.click();
  ...
  async function submit(event: FormEvent) {
    ...
    if (!data.accuracy || !data.terms) return setStatus("error");
    setStatus("saving");
  ...
  return <FieldContext.Provider value={{ data, setValue, toggle }}><main>
    ...
    <section className="content"><header><div><p className="eyebrow">PASO {step + 1} DE {steps.length}</p><div className="progress"><i style={{ width: `${completion}%` }} /></div></div><a className="admin-link" href="/admin">Panel interno</a></header>
      <form onSubmit={submit} className="card">
        ...
        {step === 5 && <><h1>Revisión y lanzamiento</h1><p className="intro">Antes de enviar, confirma la información para preparar la subcuenta y el plan de activación.</p><div className="summary"><p><b>Empresa</b>{String(data.companyName || "Pendiente")}</p><p><b>Oferta principal</b>{String(data.mainService || "Pendiente")}</p><p><b>Público</b>{asArray(data.audience).join(", ") || "Pendiente"}</p><p><b>Canales</b>{asArray(data.channels).join(", ") || "Pendiente"}</p><p><b>Integraciones</b>{asArray(data.integrations).join(", ") || "Pendiente"}</p></div><div className="grid two"><Text field="launchDate" label="Fecha objetivo de lanzamiento" placeholder="Ej. 15/09/2026" /><Text field="approvalOwner" label="Responsable de aprobación" placeholder="Nombre y cargo" /></div><label className="check"><input type="checkbox" checked={Boolean(data.accuracy)} onChange={(e) => setValue("accuracy", e.target.checked)} /> Confirmo que los datos facilitados son correctos.</label><label className="check"><input type="checkbox" checked={Boolean(data.terms)} onChange={(e) => setValue("terms", e.target.checked)} /> Autorizo el uso de estos datos para configurar la cuenta y sus automatizaciones.</label>{status === "sent" && <div className="notice success">Configuración enviada. Puedes abrir el panel interno o descargar el JSON para entregarlo a Codex.</div>}{status === "error" && <div className="notice error">Confirma las dos autorizaciones y vuelve a intentarlo. Si el envío sigue fallando, el borrador permanece guardado en este dispositivo.</div>}<button type="button" className="download" onClick={downloadConfiguration}>↓ Descargar configuración para Codex</button></>}
