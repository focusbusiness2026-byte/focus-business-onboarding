...
const helpByStep = [
  ...
  },
  {
    ...
    items: [
      ["Servicios prioritarios, precio y forma de cobro", "Selecciona uno o varios servicios prioritarios para esta prospección, indica el importe medio de una venta y cómo sueles presupuestar o cobrar."],
      ["Servicios adicionales", "Puedes añadir otros servicios que también quieras impulsar. Si eliges Otro, escribe cuál es."],
...
const initial: FormState = { audience: ["B2B"], mainService: [], services: [], sectors: [], geographies: ["España"], targetCountries: ["España"], targetClientTypes: [], channels: [], leadFields: ["Nombre", "Email", "Empresa"], integrations: [], objectives: [], automations: [], terms: false, accuracy: false, ghlPreparationAuthorization: false };
...
const options = {
  ...
  sectors: ["Tecnología", "Salud", "Industria", "Inmobiliario", "Retail / e-commerce", "Hostelería", "Educación", "Servicios profesionales", "Moda", "Otro"],
...
function normalizeDraft(saved: FormState): FormState {
  const next = { ...initial, ...saved };
  if (typeof next.mainService === "string" && next.mainService.trim()) {
    const previousService = next.mainService.trim();
    const listed = options.services.includes(previousService);
    next.mainService = [listed ? previousService : "Otro"];
    if (!listed && !String(next.mainServiceOther ?? "").trim()) next.mainServiceOther = previousService;
  }
  return next;
}
...
const labels: Record<string,string> = {
  ...
  mainService:"Servicios prioritarios", mainServiceOther:"Otro servicio prioritario", ticket:"Ticket medio", priceModel:"Modelo de precio", monthlyCapacity:"Capacidad mensual", portfolioHighlights:"Casos de éxito o portafolio", referenceCompanies:"Empresas de referencia", services:"Servicios adicionales", audience:"Público", sectors:"Sectores prioritarios", geographies:"Mercados prioritarios", targetCity:"Ciudad objetivo", targetRegion:"Región objetivo", targetRegions:"Países y regiones objetivo", targetCountries:"Países objetivo", targetClientTypes:"Tipos de cliente objetivo", idealCompanySize:"Tamaño ideal (B2B)", idealProfileDetail:"Perfil ideal detallado", decisionMaker:"Persona que suele decidir la compra", minimumBudget:"Presupuesto mínimo deseado", prospectExclusions:"Exclusiones de prospección", prospectPreferences:"Preferencias y criterios de búsqueda", servicesOther:"Otro servicio", sectorsOther:"Otro sector", targetCountriesOther:"Otro país", targetRegionsOther:"Otra región", targetClientTypesOther:"Otro tipo de cliente",
...
function Multi({ field, title, items, otherField, otherLabel, maxSelections }: { field: string; title: string; items: string[]; otherField?: string; otherLabel?: string; maxSelections?: number }) {
  ...
  const visibleTitle = field === "sectors" ? "Sectores prioritarios (selecciona hasta 3 o escribe otro)" : title;
  const visibleOtherLabel = field === "sectors" ? "Otro sector prioritario (por ejemplo: moda, deporte o turismo)" : otherLabel;
  return <details id={`field-${field}`} className={`choice-disclosure${error ? " field-error" : ""}`}><summary><span>{visibleTitle}</span><small>{values.length ? `${values.length} seleccionado${values.length === 1 ? "" : "s"}` : "Seleccionar"}</small></summary><div className="choice-panel">{effectiveMax && <p className="selection-limit">Selecciona hasta {effectiveMax}. Has elegido {values.length}.</p>}<div className="chips" aria-invalid={Boolean(error)} aria-describedby={error ? `error-${field}` : undefined}>{items.map((item) => <button type="button" className={values.includes(item) ? "chip active" : "chip"} key={item} onClick={() => context.toggle(field, item)} aria-pressed={values.includes(item)}>{item}</button>)}</div><ErrorMessage field={field} />{otherField && values.includes("Otro") && <label id={`field-${otherField}`} className={`input other-detail${otherError ? " field-error" : ""}`}><span>{visibleOtherLabel || "Especifica otra opción"}</span><input value={String(context.data[otherField] ?? "")} onChange={(event) => context.setValue(otherField, event.target.value)} placeholder="Escribe aquí…" aria-invalid={Boolean(otherError)} aria-describedby={otherError ? `error-${otherField}` : undefined} /><ErrorMessage field={otherField} /></label>}</div></details>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      if (draft) {
        try { setData(normalizeDraft(JSON.parse(draft) as FormState)); } catch { localStorage.removeItem("focus-productora-draft"); }
      }
      setDraftReady(true);
    }, 0);
  ...
  const toggle = useCallback((key: string, item: string) => {
    setData((prev) => {
      ...
      const otherField = key === "mainService" ? "mainServiceOther" : key === "services" ? "servicesOther" : key === "sectors" ? "sectorsOther" : key === "geographies" ? "geographiesOther" : key === "targetCountries" ? "targetCountriesOther" : key === "targetRegions" ? "targetRegionsOther" : key === "targetClientTypes" ? "targetClientTypesOther" : "";
  ...
  const validate = (index: number) => {
    ...
    if (index === 5) {
      ...
      if (!accessEmail) errors.contactEmail = "Añade un correo de contacto para recibir el enlace de acceso.";
    }
  ...
  return <FieldContext.Provider value={{ data, errors: fieldErrors, setValue, toggle }}><main>
    ...
    <section className="content"><header><div><p className="eyebrow">PASO {step + 1} DE {steps.length}</p><div className="progress"><i style={{ width: `${completion}%` }} /></div></div></header>
      <form onSubmit={submit} noValidate className="card">
        ...
        {step === 5 && <><h1>Revisión y preparación</h1><p className="intro">El envío crea la configuración de prospección y deja la futura subcuenta preparada para revisión. No crea GoHighLevel ni ejecuta mensajes.</p><div className="summary"><p><b>Empresa</b>{String(data.companyName || "Pendiente")}</p><p><b>Ofertas prioritarias</b>{asArray(data.mainService).map((item) => item === "Otro" && data.mainServiceOther ? String(data.mainServiceOther) : item).join(", ") || String(data.mainService || "Pendiente")}</p><p><b>Cliente objetivo</b>{asArray(data.targetClientTypes).join(", ") || "Pendiente"}</p><p><b>Países</b>{asArray(data.targetCountries).join(", ") || "Pendiente"}</p><p><b>Preparación GHL</b>Lista para validación; sin crear subcuenta</p></div><div className="grid two"><Text field="launchDate" label="Fecha objetivo de lanzamiento" placeholder="Ej. 15/09/2026" /><Text field="approvalOwner" label="Responsable de aprobación" placeholder="Nombre y cargo" /></div><section className="portal-account"><div><span className="color-label">Acceso a los portales de Focus Business</span><p>No necesitas crear una contraseña. Enviaremos al correo de contacto un enlace que caduca en 15 minutos y solo funciona una vez.</p></div><p className="portal-account-email"><b>Correo de acceso</b>{String(data.contactEmail || data.businessEmail || "Añade un correo de contacto")}</p><p className="portal-account-note">El primer uso invalida el enlace y cierra cualquier sesión anterior de ese correo. Podrás solicitar otro enlace cuando necesites volver a entrar.</p></section><div id="field-accuracy" className={`check-field${fieldErrors.accuracy ? " field-error" : ""}`}><label className="check"><input type="checkbox" checked={Boolean(data.accuracy)} onChange={(e) => setValue("accuracy", e.target.checked)} aria-invalid={Boolean(fieldErrors.accuracy)} aria-describedby={fieldErrors.accuracy ? "error-accuracy" : undefined} /> Confirmo que los datos facilitados son correctos.</label><ErrorMessage field="accuracy" /></div><div id="field-terms" className={`check-field${fieldErrors.terms ? " field-error" : ""}`}><label className="check"><input type="checkbox" checked={Boolean(data.terms)} onChange={(e) => setValue("terms", e.target.checked)} aria-invalid={Boolean(fieldErrors.terms)} aria-describedby={fieldErrors.terms ? "error-terms" : undefined} /> Autorizo el uso de estos datos exclusivamente para configurar la captación y preparar la futura subcuenta.</label><ErrorMessage field="terms" /></div><div id="field-ghlPreparationAuthorization" className={`check-field${fieldErrors.ghlPreparationAuthorization ? " field-error" : ""}`}><label className="check"><input type="checkbox" checked={Boolean(data.ghlPreparationAuthorization)} onChange={(e) => setValue("ghlPreparationAuthorization", e.target.checked)} aria-invalid={Boolean(fieldErrors.ghlPreparationAuthorization)} aria-describedby={fieldErrors.ghlPreparationAuthorization ? "error-ghlPreparationAuthorization" : undefined} /> Autorizo a Focus Business a preparar y validar estos datos para una creación posterior aprobada de la subcuenta. Entiendo que este envío no la crea ni conecta.</label><ErrorMessage field="ghlPreparationAuthorization" /></div>{status === "sent" && <div className="notice success">Configuración enviada. Revisa tu correo: recibirás un enlace de acceso válido durante 15 minutos y de un solo uso.</div>}{status === "error" && <div className="notice error">{submissionError || "No se pudo enviar la configuración. Inténtalo de nuevo; el borrador permanece guardado en este dispositivo."}</div>}</>}
