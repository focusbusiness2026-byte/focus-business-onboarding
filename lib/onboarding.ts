...
export function toSheetRecord(row: StoredSubmission): Record<string, string> {
  ...
  return {
    ...
    "Servicio prioritario": text(data.mainService), "Ticket medio": text(data.ticket), "Modelo de precio": text(data.priceModel), "Servicios": selectionWithOther(data.services, data.servicesOther), "Público": text(data.audience), "Sectores": selectionWithOther(data.sectors, data.sectorsOther), "Mercados": selectionWithOther(data.geographies, data.geographiesOther),
