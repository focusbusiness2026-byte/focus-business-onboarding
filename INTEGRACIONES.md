...
6. Tras confirmar la fila, la API puede notificar a Prospección enviando solo el `ID registro`; Prospección vuelve a leer Google Sheets, crea la configuración predeterminada y registra una ejecución pendiente o automática según exista `OPENAI_API_KEY` en su servidor.
7. Si la notificación opcional no está configurada o falla, el sondeo periódico de Prospección actúa como respaldo. El registro del formulario no se pierde ni se marca como fallido por un problema del disparador.
